class Book:
    ...
    def __init__(self, title, author, isbn):
        self.title = title # 书名（公开）
        self.author = author # 作者（公开）
        self.isbn = isbn # ISBN编号（唯一标识， 公开）
...
class Library:
    ...
    def borrow_book(self, isbn):
        ...
        for book in self.__books:
            if book.isbn == isbn:
                return book.borrow() # 调用Book类的borrow方法
        print(f"未找到ISBN为{isbn}的图书， 借出失败！ ")
        return False
    def return_book(self, isbn):
        for book in self.__books:
            if book.isbn == isbn:
                return book.return_book() # 调用Book类的return_book方法
